    ...
    <div className="flex flex-col rounded-lg border border-base-300 bg-base-100 overflow-hidden min-h-[420px]">
      <div className="bg-primary text-primary-content px-2 py-2 flex flex-wrap items-end gap-2 shrink-0">
        <div className="flex items-center gap-0.5">
          ...
          <select
            className="select select-bordered select-xs bg-primary text-primary-content border-primary-content/30 max-w-[13rem]"
            value={selectionId}
        ...
        <div className="flex flex-wrap items-end gap-2 flex-1 justify-center">
          ...
          {typeDef.toolbar === "face" ? (
            <label className="form-control">
              <span className="label-text text-[10px] text-primary-content/90">Face Type</span>
              <select
                className="select select-bordered select-xs bg-primary text-primary-content border-primary-content/30 w-[4rem]"
                value={currentRow?.faceType ?? ""}
                onChange={(e) => onFaceChange(e.target.value)}
                disabled={!sortedRows.length}
              >
                {faceOptions.map((f) => (
                  <option key={f} value={f} className="text-base-content bg-base-100">
                    {f}
                  </option>
                ))}
              </select>
            ...
            <label className="form-control">
              <span className="label-text text-[10px] text-primary-content/90">Actuator</span>
              <select
                className="select select-bordered select-xs bg-primary text-primary-content border-primary-content/30 w-[5.5rem]"
                value={currentRow?.actuator ?? ""}
                onChange={(e) => onActuatorChange(e.target.value)}
                disabled={!sortedRows.length}
              >
                {actuatorOptions.map((a) => (
                  <option key={a} value={a} className="text-base-content bg-base-100">
                    {a}
                  </option>
                ))}
              </select>
