...
function effectiveLineWps(line) {
  if (!line) return "";
  if (line.wpsOverridesSystem === false) return "";
  if (line.wpsOverridesSystem === true) return (line.wps ?? "").trim();
  return (line.wps ?? "").trim();
}

function effectiveSystemWps(system) {
  if (!system) return "";
  if (system.wpsOverridesProject === false) return "";
  if (system.wpsOverridesProject === true) return (system.wps ?? "").trim();
  return (system.wps ?? "").trim();
}

/**
 * WPS from line, then parent system, then project defaultWps (ignores weld.wps). For UI “inherited” hints.
 */
export function getInheritedWpsCode(weld, systems, lines, spools, drawingSettings) {
  const lineId = getWeldLineId(weld, spools);
  const line = lineId ? (lines || []).find((x) => x.id === lineId) : null;
  const lineWps = effectiveLineWps(line);
  if (lineWps) return lineWps;
  const system = line?.systemId ? (systems || []).find((s) => s.id === line.systemId) : null;
  const sysWps = effectiveSystemWps(system);
  if (sysWps) return sysWps;
  return (drawingSettings?.defaultWps ?? "").trim();
}

/**
 * Where {@link getInheritedWpsCode} gets its value from (line wins over system over project).
 * @returns {"line"|"system"|"project"|null}
 */
export function getInheritedWpsSource(weld, systems, lines, spools, drawingSettings) {
  const lineId = getWeldLineId(weld, spools);
  const line = lineId ? (lines || []).find((x) => x.id === lineId) : null;
  const lineWps = effectiveLineWps(line);
  ...
  const proj = (drawingSettings?.defaultWps ?? "").trim();
  if (proj) return "project";
  return null;
}

/**
 * Effective WPS: explicit weld.wps if set, else line, else system, else project defaultWps.
 */
export function getResolvedWpsCode(weld, systems, lines, spools, drawingSettings) {
  const direct = (weld?.wps ?? "").trim();
  if (direct) return direct;
  return getInheritedWpsCode(weld, systems, lines, spools, drawingSettings);
}

/**
 * WPS inherited by a line before applying line.wps (system effective + project default).
 */
export function getInheritedWpsBeforeLine(system, drawingSettings) {
  const sysWps = effectiveSystemWps(system);
  if (sysWps) return sysWps;
  return (drawingSettings?.defaultWps ?? "").trim();
}
 ...
 */
export function getUnregisteredWpsUsageGroups(weldPoints, wpsLibrary, systems, lines, spools, drawingSettings = {}) {
