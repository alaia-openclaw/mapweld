...
/**
 * Effective WPS strings on welds that do not match any library row (code / title / effective).
...
export function getUnregisteredWpsUsageGroups(weldPoints, wpsLibrary, systems, lines, spools) {
  ...
  for (const w of welds) {
    ...
    if (findWpsLibraryEntriesMatchingUserText(lib, resolved).length > 0) continue;
