/**
 * Socketwelded valves — reference dimensions for catalog UI (Pipedata-style).
 * Representative data for UI; verify against standards / vendors for procurement.
 */

export const SOCKETWELDED_VALVE_DNS = [15, 20, 25, 32, 40, 50, 65, 80, 100];
export const SOCKETWELDED_VALVE_PRESSURE_CLASSES = ["150#", "300#", "600#", "800#"];

const DNS = SOCKETWELDED_VALVE_DNS;
const PRESSURE_CLASSES = SOCKETWELDED_VALVE_PRESSURE_CLASSES;

export const SOCKETWELDED_VALVE_TYPES = [
  {
    id: "gate",
    selectionId: "valves-socketwelded-gate",
    label: "Socketwelded Gate Valve",
    referenceStandard: "ASME Unadopted",
  },
  {
    id: "globe",
    selectionId: "valves-socketwelded-globe",
    label: "Socketwelded Globe Valve",
    referenceStandard: "ASME Unadopted",
  },
  {
    id: "horizontal-check",
    selectionId: "valves-socketwelded-horizontal-check",
    label: "Horizontal Check Valve",
    referenceStandard: "ASME Unadopted",
  },
  {
    id: "vertical-check",
    selectionId: "valves-socketwelded-vertical-check",
    label: "Vertical Check Valve",
    referenceStandard: "ASME Unadopted",
  },
  {
    id: "ball",
    selectionId: "valves-socketwelded-ball",
    label: "Socketwelded Ball Valve",
    referenceStandard: "ASME Unadopted",
  },
];
...
const ALL_ROWS = SOCKETWELDED_VALVE_TYPES.flatMap((t) => buildRowsForValveType(t.id));

export function getSocketweldedValveRowsForType(valveTypeId) {
  return ALL_ROWS.filter((r) => r.valveTypeId === valveTypeId);
}

export function getSocketweldedValveTypeBySelectionId(selectionId) {
  return SOCKETWELDED_VALVE_TYPES.find((t) => t.selectionId === selectionId) || null;
}

export function matchSocketweldedValveRow(row, search) {
...
export function buildSocketweldedValveTitle(typeDef, row) {
