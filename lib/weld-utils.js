...
export function getSelectedWeldIdsForNdtMethod(weldPoints, drawingSettings, method) {
  ...
  if (pctShop >= 100 && pctField >= 100) return null;
...
export function computeNdtSelection(weld, drawingSettings = {}, weldPoints = []) {
  ...
  const methods = getMethodListForWeld(weld, drawingSettings);
  if (ndtRequired === "no") {
    const off = {};
    methods.forEach((m) => { off[m] = false; });
    return off;
  }
  ...
  methods.forEach((m) => {
    ...
    const pct = getPctForMethod(ndtRequirements, m, location);
    if (pct >= 100) {
      selection[m] = true;
      return;
    }
