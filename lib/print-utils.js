...
function buildWeldMapHtml(weldPoints, { pdfFilename, spools, personnel, drawingSettings }) {
  ...
  const allHeaders = WELD_MAP_HEADERS;
...
function buildProjectProgressHtml(weldPoints, spools, drawingSettings, getWeldName) {
  ...
  safeWelds.forEach((w) => {
    const ndtSel = computeNdtSelection(w, drawingSettings, safeWelds);
...
export async function runPrint({
  ...
  if (pdfDrawing && pdfBlob) {
    const includeMarkers = markers && (markers.welds || markers.spools || markers.parts);
    if (includeMarkers) {
      ...
      try {
        if (target) {
          ...
          const canvas = await html2canvas(target, {
            ...
            logging: false,
            backgroundColor: "#fff",
            ignoreElements: (el) => el.closest("[data-print-hide]") !== null,
          });
          const dataUrl = canvas.toDataURL("image/png");
          html += '<div class="print-page print-page-pdf"><h2>PDF drawing with markers</h2><div class="print-content">';
          html += `<img src="${dataUrl}" alt="Drawing" />`;
          html += "</div></div>";
        } else {
          html += '<div class="print-page print-page-pdf"><h2>PDF drawing</h2><div class="print-content">';
          html += `<iframe src="${typeof pdfBlob === "string" ? pdfBlob : URL.createObjectURL(pdfBlob)}" style="width:100%;height:600px;border:1px solid #ccc;" title="PDF"></iframe>`;
          html += "</div></div>";
        }
      } catch (err) {
        console.warn("Failed to capture PDF overlay:", err);
        html += '<div class="print-page print-page-pdf"><h2>PDF drawing</h2><div class="print-content">';
        html += `<iframe src="${typeof pdfBlob === "string" ? pdfBlob : URL.createObjectURL(pdfBlob)}" style="width:100%;height:600px;border:1px solid #ccc;" title="PDF"></iframe>`;
        html += "</div></div>";
      } finally {
  ...
  if (projectProgress) {
    ...
    html += buildProjectProgressHtml(weldPoints, spools, drawingSettings, getWeldName);
    html += "</div>";
  }
