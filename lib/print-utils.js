...
export async function runPrint({
  ...
  function drawOutlinedLabel(ctx, { x, y, text, color, fontSize = 12, shape = "pill" }) {
    const padX = 8;
    const padY = 5;
  ...
  function drawFieldWeldMark(ctx, x, y, color, arm, lineW) {
    ctx.strokeStyle = color;
    ctx.lineWidth = lineW;
    ctx.beginPath();
    ...
    if (exportMarkerLayers?.welds !== false) {
      (weldPoints || [])
        ...
        .forEach((w) => {
          ...
          const markArm = Math.max(2.5, strokeW * 2.2);
          const dotR = Math.max(2.2, strokeW * 1.8);
          const dotOutline = Math.max(1.1, strokeW * 0.95);
          ...
          if (isField)
            drawFieldWeldMark(ctx, pWeld.x, pWeld.y, lineCol, markArm, Math.max(1.2, strokeW));
