...
function buildWeldMapSection({
  ...
  const { rows = [], maxWelders = 1, ndtMethods } = buildExportRows(weldPoints, {
  ...
  const head = buildWeldsSheetHeaders(maxWelders, ndtMethods);
  const body = rows
  ...
  return `
    <section class="page">
      ...
      <table>
        ...
        <tbody>${body || `<tr><td colspan='${Math.max(head.length, 1)}'>No welds</td></tr>`}</tbody>
      </table>
    </section>
  `;
}
