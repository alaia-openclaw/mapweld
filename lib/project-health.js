...
import { computeNdtSelection, getWeldSectionCompletion, getMethodListForWeld } from "./weld-utils";
import {
  isWeldJointDimensionallyComplete,
  weldHasBothPartsLinked,
} from "./joint-dimensions";
import { getResolvedNdtRequirementsForWeld } from "./ndt-resolution";
import { getResolvedWpsCode } from "./wps-resolution";
import { NDT_OVERRIDE_OPTIONS, NDT_METHOD_LABELS } from "./constants";
...
/**
 * Weld has any non-empty welding record field (user started welding data).
 * @param {object} weld
 */
function weldHasWeldingProgress(weld) {
  const records = weld?.weldingRecords || [];
  return records.some((rec) => {
    if (!rec) return false;
    if ((rec.date || "").trim()) return true;
    if ((rec.welderName || "").trim()) return true;
    if ((rec.welderIds || []).length > 0) return true;
    if ((rec.weldingProcesses || []).length > 0) return true;
    const nums = rec.electrodeNumbers || [];
    return nums.some((x) => (x || "").trim());
  });
}

/**
 * Any NDT result or outcome stored on the weld.
 * @param {object} weld
 */
function weldHasNdtRecorded(weld) {
  const results = weld?.ndtResults || {};
  if (Object.values(results).some(Boolean)) return true;
  const outcomes = weld?.ndtResultOutcome || {};
  return Object.keys(outcomes).length > 0;
}

/**
 * User entered any fit-up field (partial fit-up intent).
 * @param {object} weld
 */
function weldHasFitupProgress(weld) {
  if ((weld?.fitterName || "").trim()) return true;
  if ((weld?.dateFitUp || "").trim()) return true;
  if ((weld?.heatNumber1 || "").trim()) return true;
  if ((weld?.heatNumber2 || "").trim()) return true;
  return false;
}
...
export function collectProjectHealthIssues(snapshot = {}) {
  ...
  const ndtContext = { systems, lines, spools, parts };
  ...
  if (!(projectMeta.projectName || "").trim()) {
    issues.push({
      ...
      detail: "Add a project name under Settings → Project info & libraries for clearer exports and file names.",
    });
  }
  if (!(projectMeta.client || "").trim()) {
    issues.push({
      ...
      detail: "Optional: set client in Settings → Project info & libraries.",
    });
  }
  if (!(projectMeta.spec || "").trim()) {
    issues.push({
      ...
      detail: "Optional: set spec / standard in Settings → Project info & libraries.",
    });
  }
  ...
  if (!Array.isArray(ndtReqs) || ndtReqs.length === 0) {
    issues.push({
      ...
      detail: "Settings → Project NDT & spec has no methods. Weld NDT expectations may be unclear.",
    });
  }
  if (!(drawingSettings.weldingSpec || "").trim()) {
    issues.push({
      ...
      detail: "Optional: add welding specification under Settings → Project NDT & spec.",
    });
  }

  const needsFitterPersonnel = weldPoints.some((w) => {
    const ndtSel = computeNdtSelection(w, drawingSettings, weldPoints, ndtContext);
    const s = getWeldSectionCompletion(w, ndtSel, ndtContext);
    return s.fitup || weldHasFitupProgress(w);
  });
  const needsWelderPersonnel = weldPoints.some((w) => {
    const ndtSel = computeNdtSelection(w, drawingSettings, weldPoints, ndtContext);
    const s = getWeldSectionCompletion(w, ndtSel, ndtContext);
    return s.welding || weldHasWeldingProgress(w);
  });
  if (needsFitterPersonnel && !(personnel.fitters || []).length) {
    issues.push({
      id: "general::noFitters",
      category: "general",
      severity: "warning",
      title: "No fitters in personnel",
      detail: "At least one weld has fit-up data — add fitters under Settings → Personnel for consistency.",
    });
  }
  if (needsWelderPersonnel && !(personnel.welders || []).length) {
    issues.push({
      id: "general::noWelders",
      category: "general",
      severity: "warning",
      title: "No welders in personnel",
      detail: "At least one weld has welding data — add welders under Settings → Personnel for consistency.",
    });
  }
  ...
  weldPoints.forEach((w) => {
    ...
    const part1 = w?.partId1 ? parts.find((p) => p?.id === w.partId1) : null;
    const part2 = w?.partId2 ? parts.find((p) => p?.id === w.partId2) : null;
    if (weldHasFitupProgress(w) || section.welding) {
      if (!weldHasBothPartsLinked(w)) {
        issues.push({
          id: `welds::missingParts::${wid}`,
          category: "welds",
          severity: "warning",
          title: "Weld missing Part 1 / Part 2",
          detail: "Link both parts on the weld (fit-up) for joint traceability.",
          entityType: "weld",
          entityId: wid,
        });
      } else if (!isWeldJointDimensionallyComplete(w, part1, part2)) {
        issues.push({
          id: `welds::jointDims::${wid}`,
          category: "welds",
          severity: "warning",
          title: "Joint NPS or wall thickness incomplete",
          detail:
            "Each side needs NPS (diameter) and schedule / wall (e.g. Sch 40 or catalog thickness). Set on the parts or use weld overrides in fit-up.",
          entityType: "weld",
          entityId: wid,
        });
      }
    ...
    const executionStarted =
      section.fitup ||
      section.welding ||
      weldHasFitupProgress(w) ||
      weldHasWeldingProgress(w) ||
      weldHasNdtRecorded(w);

    // WPS: only once execution / traceability data exists (stubs on the drawing are OK)
    if (!section.general && executionStarted) {
      issues.push({
        id: `welds::noWps::${wid}`,
        category: "welds",
        severity: "warning",
        title: "Weld missing WPS",
        detail: "Set a WPS on the weld (General section) when recording fit-up, welding, or NDT.",
        entityType: "weld",
        entityId: wid,
      });
    }

    // Fit-up required before welding can be coherent
    if (!section.fitup && (weldHasWeldingProgress(w) || section.welding)) {
      issues.push({
        id: `welds::fitup::${wid}`,
        category: "welds",
        severity: "warning",
        title: "Welding without complete fit-up",
        detail: "Fit-up must be registered (fitter, date, heat number) before welding data.",
        entityType: "weld",
        entityId: wid,
      });
    }

    // Started welding rows but not all records valid
    if (weldHasWeldingProgress(w) && !section.welding) {
      issues.push({
    ...
    // NDT cannot precede completed welding
    if (weldHasNdtRecorded(w) && !section.welding) {
      issues.push({
        id: `welds::ndtBeforeWelding::${wid}`,
        category: "welds",
        severity: "warning",
        title: "NDT recorded before welding is complete",
        detail: "Remove or complete welding records first; NDT should follow finished welding.",
        entityType: "weld",
        entityId: wid,
      });
    }

    // Library: WPS (effective code includes line/system inheritance)
    const wpsTrim = getResolvedWpsCode(w, systems, lines, spools);
    if (hasWpsLibrary && wpsTrim && !wpsCodesLower.has(wpsTrim.toLowerCase()) && executionStarted) {
      issues.push({
        id: `library::wps::${wid}`,
        category: "library",
        severity: "warning",
        title: "WPS not in project library",
        detail: `WPS "${wpsTrim}" is not listed in the WPS library (Settings → Project info & libraries).`,
        entityType: "weld",
        entityId: wid,
      });
    }
    ...
    if (
      section.fitup &&
      fitterTrim &&
      fitterNamesLower.size > 0 &&
      !fitterNamesLower.has(fitterTrim.toLowerCase())
    ) {
      issues.push({
        ...
        detail: `Fitter "${fitterTrim}" does not match any Settings → Personnel fitter name.`,
        entityType: "weld",
        entityId: wid,
      });
    }
    ...
    (w.weldingRecords || []).forEach((rec, recIdx) => {
      ...
      nums.forEach((code) => {
        ...
        if (
          hasElectrodeLibrary &&
          c &&
          !electrodeCodesLower.has(c.toLowerCase()) &&
          (weldHasWeldingProgress(w) || section.welding)
        ) {
          issues.push({
            ...
            detail: `"${c}" is not listed in the electrode library (Settings → Project info & libraries).`,
            entityType: "weld",
            entityId: wid,
          });
        }
  ...
  weldPoints.forEach((w) => {
    ...
    const resolvedReqs = getResolvedNdtRequirementsForWeld(w, drawingSettings, systems, lines, spools);
    const methods = getMethodListForWeld(w, drawingSettings, resolvedReqs);
    methods.forEach((method) => {
      const needDone =
        overrides[method] === NDT_OVERRIDE_OPTIONS.REQUIRED ||
        (overrides[method] !== NDT_OVERRIDE_OPTIONS.EXEMPT && ndtSel[method]);
      if (!needDone) return;
      // Only flag missing NDT once welding is complete — otherwise it's not yet applicable
      if (!section.welding) return;
      if (!results[method]) {
