...
export function collectProjectHealthIssues(snapshot = {}) {
  ...
  const ndtContext = { systems, lines, spools, parts, drawingSettings };
  ...
  weldPoints.forEach((w) => {
    ...
    if (weldHasFitupProgress(w) || section.welding) {
      if (!weldHasBothPartsLinked(w)) {
        ...
        issues.push({
          ...
          category: "welds",
          severity: "warning",
          title: "Joint NPS or schedule incomplete",
          detail:
            "Each side needs NPS (diameter) and schedule (e.g. Sch 40). Set on the parts or use weld overrides in fit-up.",
          entityType: "weld",
          entityId: wid,
        });
      }
    ...
    const wpsTrim = getResolvedWpsCode(w, systems, lines, spools, drawingSettings);
