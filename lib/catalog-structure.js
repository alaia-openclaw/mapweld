...
/** Pipe schedule designation only (e.g. STD, 40); not wall thickness in mm. */
function getPipeScheduleFromAttributes(a) {
  if (!a || typeof a !== "object") return undefined;
  const raw = a.schedule ?? a.Schedule ?? a.Sch;
 ...
 * Used for pipe, butt-weld fittings (per-file schedule), and flange bore (CSV Schedule column or row key).
 */
...
function pipeScheduleFromFlangeAttributes(a) {
  if (!a || typeof a !== "object") return undefined;
  for (const key of Object.keys(a)) {
    const lk = key.toLowerCase().replace(/\s+/g, " ");
    if (
      /^(schedule|sch|pipe schedule|pipe sch)$/.test(lk) ||
      (lk.includes("schedule") && !/wall|thk|thick/i.test(lk))
    ) {
      const v = a[key];
 ...
 * Does not use flange neck or plate thickness in mm (those are not pipe schedules).
 */
export function getFlangePipeScheduleDisplay(row) {
  const a = row.attributes || {};
  const fromExplicit = getPipeScheduleFromAttributes(a);
  if (fromExplicit) return fromExplicit;
  const fromKeys = pipeScheduleFromFlangeAttributes(a);
