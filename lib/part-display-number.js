/** Keep first occurrence when the same part id appears more than once in the array. */
export function dedupePartsById(parts) {
  const seen = new Set();
...
export function partSpecKey(part) {
  ...
  const mat = String(part.materialGrade ?? "").trim();
  const len = String(part.length ?? "").trim();
  return `m:${cat}|${pt}|${nps}|${th}|${vari}|${mat}|${len}`;
}
...
export function assignPartDisplayNumbersForAllDrawings(parts, partMarkers) {
  const safeParts = dedupePartsById(Array.isArray(parts) ? parts : []);
