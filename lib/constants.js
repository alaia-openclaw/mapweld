...
export function sortNdtMethods(methods = []) {
  return [...new Set(methods.filter(Boolean))]
    .sort((a, b) => {
      const ai = NDT_METHODS.indexOf(a);
      const bi = NDT_METHODS.indexOf(b);
      const aKnown = ai !== -1;
      const bKnown = bi !== -1;
...
/** NDT result outcome (per weld, from report) */
export const NDT_RESULT_OUTCOMES = {
  ACCEPTED: "accepted",
  REJECTED: "rejected",
  OMITTED_OR_INCONCLUSIVE: "omitted_or_inconclusive",
};

export const NDT_RESULT_OUTCOME_LABELS = {
  [NDT_RESULT_OUTCOMES.ACCEPTED]: "Accepted",
  [NDT_RESULT_OUTCOMES.REJECTED]: "Rejected",
  [NDT_RESULT_OUTCOMES.OMITTED_OR_INCONCLUSIVE]: "Omitted or inconclusive",
  // Legacy (still in stored data)
  reject: "Rejected",
  repair: "Repair needed",
};

/** NDT request status */
export const NDT_REQUEST_STATUS = {
  DRAFT: "draft",
  SENT: "sent",
  COMPLETED: "completed",
};

export const NDT_REQUEST_STATUS_LABELS = {
  [NDT_REQUEST_STATUS.DRAFT]: "Draft",
  [NDT_REQUEST_STATUS.SENT]: "Sent",
  [NDT_REQUEST_STATUS.COMPLETED]: "Completed",
};

/** NDT report status */
export const NDT_REPORT_STATUS = {
  CREATED: "created",
  COMPLETED: "completed",
};

export const NDT_REPORT_STATUS_LABELS = {
  [NDT_REPORT_STATUS.CREATED]: "Created",
  [NDT_REPORT_STATUS.COMPLETED]: "Completed",
};
...
/** Part types (components of a spool) */
...
export const PART_TYPE_LABELS = {
  [PART_TYPES.PIPE]: "Pipe",
  [PART_TYPES.ELBOW]: "Elbow",
  [PART_TYPES.TEE]: "Tee",
  [PART_TYPES.REDUCER]: "Reducer",
  [PART_TYPES.CAP]: "Cap",
  [PART_TYPES.FLANGE]: "Flange",
  [PART_TYPES.COUPLING]: "Coupling",
  [PART_TYPES.OTHER]: "Other",
};
...
export function formatNdtRequirements(ndtRequirements = []) {
  ...
  return ndtRequirements
    .map((r) => {
      const s = r.pctShop ?? r.pct ?? 100;
      const f = r.pctField ?? r.pct ?? 100;
      if (s === f) return `${s}% ${r.method}`;
      return `Shop ${s}% / Field ${f}% ${r.method}`;
    })
    .join(", ");
}
