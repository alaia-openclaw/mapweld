...
import { SOCKETWELDED_VALVE_TYPES, getSocketweldedValveRowsForType } from "@/lib/socketwelded-valves-data";
...
export function buildVirtualValveEntriesForLeaf(leafId) {
  ...
  const st = SOCKETWELDED_VALVE_TYPES.find((t) => t.selectionId === leafId);
  if (st) {
    return getSocketweldedValveRowsForType(st.id).map((row) => simpleRowToEntry(st, row));
  }
  return [];
}
