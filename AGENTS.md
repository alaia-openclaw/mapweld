...
- **`.weldproject` version** is `PROJECT_FILE_VERSION` in `lib/project-file.js` (v6: `ndtRequirements` on systems/lines; v7: weld `jointDimensions` for NPS/schedule per side with part inheritance — `lib/joint-dimensions.js`, fit-up completion requires both parts + effective dimensions).
